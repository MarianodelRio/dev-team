...
- State transitions go through the canonical scripts in `scripts/` — `dt-claim`, `dt-ready`, `dt-pr`, `dt-done`, `dt-cancel`, `dt-restart` — not hand-rolled git. They are idempotent, validate inputs, and make the branch push the atomic claim lock. `dt-verify` runs the configured quality checks. `dt-board` regenerates the `.dt-index.json` cache (git-ignored)
...
9. **Never modify `spec.md` directly** — use `/refine`. It classifies affected tasks by state and propagates changes safely.
