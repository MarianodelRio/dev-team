...
**Sub-agent spawn depth** — the review-coordinator spawns reviewers from inside a sub-agent, which
needs depth 2. If `.claude/settings.json` is absent, or its `env` lacks
`CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH`, print:
```
⚠️  WARNING: .claude/settings.json does not set CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH: "2" —
    the Phase 4 review pipeline may not be able to launch its sub-reviewers.
```
...
Four model slots, each can be any Claude model or an alternative:
  ...
  implementation → writing code, implementing tasks, reviewing PRs
                   (applies to: planner.md, coder.md, review-coordinator.md,
                    code-quality.md, adversarial.md, security.md,
                    smoke-tester.md, mutation-tester.md, spec-coverage.md)
  Current: [value]
